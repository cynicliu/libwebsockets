...
display_update(lws_display_state_t *lds)
{
	...
	fs_update_html->len = lws_snprintf(p, 1024,
		...
		"<style> body { font-family: sun; font-size: 24pt; background-color: #fff; }"
		...
		".bar { display: inline; width: 310px; height: 30px; color: #fff; background-color: #000; margin-left: auto; margin-right:auto; padding-left:4px; padding-right:4px; padding-top:4px; padding-bottom: 4px; margin-bottom: 48px }"
		".done { background-color: #fff; color: #000; width: %dpx; height: 20px; padding-top: 1px; font-size: 20pt; text-align: center }"
		//".todo { background-color: #fff; color: #000; height: 20px }"
		"</style><body><h1>UPDATING</h1>"
...
display_splash(lws_display_state_t *lds)
{
	...
	fs_splash_html = malloc(sizeof(*fs_splash_html) + 1024);
	if (!fs_splash_html)
		return 0;
	...
	fs_splash_html->data = p = (char *)(fs_splash_html + 1);
	...
	fs_splash_html->len = lws_snprintf(p, 1024,
		"<!doctype html><html><head><meta charset=\"utf-8\" /></head><style>"
		"body { font-family: sun; font-size: 24pt; color:#000"
	//	"background-image: \"splash.jpg\"; width: %dpx; height: %dpx"
		"}"
		"h1 { font-family: sans; font-size: 32px; text-align: center; margin-bottom: 12px; color:#000; }"
		".bar { display: inline; width: 310px; height: 30px; color: #fff; background-color: #000; margin-left: auto; margin-right:auto; padding-left:4px; padding-right:4px; padding-top:4px; padding-bottom: 4px; margin-bottom: 48px }"
		".done { background-color: #fff; color: #000; width: %dpx; height: 20px; padding-top: 1px; font-size: 20pt; text-align: center }"
		"</style><body><h1>Booting</h1>"
		"<div class=\"bar\" id=\"progress\"><div class=\"done\">%d%%</div></div><br>"
		"%s<br>"
		"Fw: %s"
		"</body></html>",
		(boot_step * 300) / 13,
...
//				lds->disp->ic.wh_px[1].whole,
	...
	lws_dlo_file_register(cx, fs_splash_html);
...
//		lws_display_render_add_id(&rs, "progress", NULL);
	...
	lwsl_err("%s: starting splash\n", __func__);
	init_browse(cx, &rs, "file://dlofs/splash.html");
...
display_completion_cb(lws_display_state_t *lds, int a)
{
	...
	case 2: /* display update finished */
		...
		if (fs_splash_html) {
			lws_dlo_file_unregister(&fs_splash_html);
			lws_display_state_active(lds);
		}
